const miModulo = (() => {
  ...
  Array.from(divCartasJugadores).reverse();
        
        
  const modalBody =  document.querySelector('#modalBody');

  const showModal = (ganador) => {
    let myModal = new bootstrap.Modal(document.getElementById('exampleModal'), {
      keyboard: false
    })
    modalBody.innerText = `${ganador}`
    myModal.show()
  }
  ...
  const determinarGanador = () => {
      ...
      setTimeout(() => {
          if( puntosComputadora === puntosMinimos ) {
              ...
              showModal('¡Computadora gana!')
          } else if( puntosComputadora > 21 ) {
              // alert('Jugador Gana');
              showModal('¡Jugador gana!')
          } else {
              // alert('Jugador Gana')
              showModal('¡Jugador gana!')
          }
  ...
  const turnoComputadora = ( puntosMinimos ) => {
      ...
      do {
          ...
          puntosComputadora = acumularPuntos(carta, puntosJugadores.length - 2 );
